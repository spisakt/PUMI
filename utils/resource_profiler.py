...
def log_to_dict(logfile):
    ...
    for n in nodes_list:
        n['start'] = datetime.datetime.strptime(n['start'], format)
        n['finish'] = datetime.datetime.strptime(n['finish'], format)
        if n['runtime_threads']=="N/A":
            n['runtime_threads'] = 1;
        if n['runtime_memory_gb'] == "N/A":
            n['runtime_memory_gb'] = 0.2;
